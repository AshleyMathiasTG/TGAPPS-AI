...
# -------------------------
# DOCUMENT TEXT EXTRACTION (PDF, DOCX, DOC)
# -------------------------
...
def extract_text_from_docx(docx_path):
    ...
    try:
        ...
        if not doc.paragraphs:
            raise ValueError(f"DOCX file '{docx_path}' has no content")
        ...
        if not text_parts:
            raise ValueError(f"DOCX file '{docx_path}' has no extractable text")
...
def extract_text_from_document(doc_path):
    ...
    Raises:
        ...
        ValueError: If file type not supported or file is empty
    ...
    if ext == '.pdf':
        try:
            with pdfplumber.open(doc_path) as pdf:
                if not pdf.pages:
                    raise ValueError(f"PDF file '{doc_path}' has no pages")
...
def extract_text_from_pdf(pdf_path):
    ...
    Use extract_text_from_document() for automatic file type detection.
    """
...
if __name__ == "__main__":
    doc_path = "Resume/Resume5.pdf"  
    jd_path = "jds/program_manager.txt" 

    try:
        # Validate input
        if not doc_path:
            print("Error: Document path is required", file=sys.stderr)
            sys.exit(1)

        if not os.path.exists(doc_path):
            print(f"Error: Resume document not found: {doc_path}", file=sys.stderr)
            sys.exit(1)
        
        # Parse resume
        print(f"Processing resume: {doc_path}", file=sys.stderr)
        if jd_path:
            print(f"Applying JD filtering from: {jd_path}", file=sys.stderr)

        parsed_data = parse_resume(doc_path, jd_path)
