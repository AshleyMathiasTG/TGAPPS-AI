...
result = parse_resume("Resume/resume9.pdf")
print(result)
```
...
3. Update the `pdf_path` and `jd_path` variables in `main.py` (lines 584-585)
4. Run the parser:
```bash
python main.py
```
...
**Purpose**: Extract all text content from a PDF file  
**Parameters**: 
...
Uses `pdfplumber` library to open PDF and extract text from all pages, handling empty pages gracefully.
...
#### `parse_resume(pdf_path: str, jd_path: str | None = None) -> dict`
